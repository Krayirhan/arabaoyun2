...
function updateMissionUI() {
  ...
  const distance = Math.min(
    500,
    Math.floor(missionState.distance + runDistance),
  );
...
function updatePowerUI() {
  ...
  powerEl.textContent = powers.length
    ? powers.join("  \xB7  ")
    : "G\xDC\xC7LEND\u0130R\u0130C\u0130 YOK";
}
...
function toggleAtmosphere() {
  ...
  if (nightMode) {
    ...
    s.fog.color.set(7973311);
    sun.color.set(16764830);
    sun.intensity = 3;
  ...
  new GLTFLoader().load(
    ...
    (gltf) => {
      ...
      if (gltf.animations.length) {
        ...
        let clip =
          gltf.animations.find((a) => a.name.includes("Flying")) ||
          gltf.animations[0];
  ...
  [
    ...
    l.load(
      ...
      (gltf) => {
        ...
        buildingTpls.push({ scene: m, w: size.x, h: size.y, d: size.z });
      },
      void 0,
      (error) => {
        window.dispatchEvent(
          new CustomEvent("asseterror", { detail: { path, error } }),
        );
      },
    ),
  );
    ...
    l.load(
      ...
      (g) => {
        ...
        for (let i = 0; i < TREE_COUNT; i++) {
          ...
          o.userData.baseX =
            side * (TREE_SIDE_MIN + Math.random() * TREE_SIDE_RANGE);
...
function seedCityProps() {
  ...
  for (let z = 1300, index = 0; z > -1300; z -= 90, index += 1) {
    ...
    const car = (
      index % 3 === 0 ? cityTemplates.taxi : cityTemplates.car
    ).clone(true);
...
function recycleTrees() {
  trees.forEach((o) => {
    if (o.position.z > dragon.position.z + 30) {
      ...
      o.position.x =
        (Math.random() > 0.5 ? 1 : -1) *
        (TREE_SIDE_MIN + Math.random() * TREE_SIDE_RANGE);
...
function buildRoad() {
  ...
  for (const x of [-9, 9]) {
    const edge = new THREE.Mesh(
      new THREE.BoxGeometry(0.55, 0.03, 3e3),
      lanePaint,
    );
  ...
  for (let z = -12; z > -2980; z -= 28) {
    for (const x of [-5, 0, 5]) {
      const marker = new THREE.Mesh(
        new THREE.BoxGeometry(0.6, 0.035, 16),
        lanePaint,
      );
...
function build() {
  ...
  new THREE.TextureLoader().load(
    "assets/skyboxes-pack/Skyboxes/skybox-day.png",
    (t) => {
...
const DRAG_HX = DRAGON_COLLIDER.x,
  ...
  GROUND_Y = 0;
function addBuildingAt(x, z, rotation) {
  if (!buildingTpls.length) return;
  let tpl = buildingTpls[Math.floor(Math.random() * buildingTpls.length)],
    targetFootprint = 7 + Math.random() * 3,
    scale = targetFootprint / Math.max(tpl.w, tpl.d, 0.001),
...
function spawnObstaclePattern() {
  ...
  pattern.forEach(({ lane, offset = 0 }, index) => {
    addBuildingAt(
      lane * LANE_WIDTH,
      baseZ + offset,
      ((patternIndex + index) % 2) * (Math.PI / 2),
    );
  });
...
function addCloud() {
  ...
  g.position.set(
    (Math.random() - 0.5) * 50,
    16 + Math.random() * 10,
    dragon.position.z - 140 - Math.random() * 100,
  );
...
function end() {
  ...
  missionState.buildings = Math.min(
    3,
    missionState.buildings + passedBuildings,
  );
